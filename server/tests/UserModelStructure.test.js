...
console.log('\n🔍 Validations:');
console.log('  - username required:', schema.paths.username.isRequired ? '✅' : '❌');
console.log('  - email required:', schema.paths.email.isRequired ? '✅' : '❌');
console.log('  - email unique:', schema.paths.email._index?.unique ? '✅' : '❌');
console.log('  - password required:', schema.paths.password.isRequired ? '✅' : '❌');
...
try {
    ...
    if (hooks) {
        ...
        console.log('  - Hooks présents: ✅ Oui (structure différente dans cette version)');
    }
} catch (error) {
